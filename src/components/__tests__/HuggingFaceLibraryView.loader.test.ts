...
function stubFetch(returnValue: HfModel[] = []): { calls: string[]; fn: ReturnType<typeof vi.fn> } {
  const calls: string[] = [];
...
describe("loadHuggingFace", () => {
  ...
  it("emits a single fetch with no params when filters are empty", async () => {
    ...
    await loadHuggingFace({
      query: "", tasks: [], libraries: [], inference: false,
      sort: "trending", offset: 0, signal: ctrl.signal,
    });
  ...
  it("fans out to N parallel fetches when multiple tasks are picked, capped at 10", async () => {
    const { calls } = stubFetch();
    const ctrl = new AbortController();
    const tasks = ["text-generation", "text-to-image", "text-to-speech", "image-to-text"];
    await loadHuggingFace({
      query: "qwen", tasks, libraries: ["mlx", "gguf"], inference: true,
      sort: "downloads", offset: 0, signal: ctrl.signal,
    });
    expect(calls.length).toBe(tasks.length);
    for (const t of tasks) {
      expect(calls.some((u) => u.includes(`pipeline_tag=${encodeURIComponent(t)}`))).toBe(true);
    }
...
describe("client-side filter helpers", () => {
  const M = (id: string, tags: string[] = [], numParameters?: number): HfModel => ({
    id, downloads: 0, likes: 0, tags, numParameters,
  });

  it("matchesApps detects both bare and library-prefixed tags", () => {
    expect(matchesApps(M("a", ["vllm"]), ["vllm"])).toBe(true);
    expect(matchesApps(M("a", ["library:llama.cpp"]), ["llama.cpp"])).toBe(true);
  ...
  it("matchesProviders honors provider:* and inference_provider:* prefixes", () => {
    ...
    expect(matchesProviders(M("a", ["inference_provider:together"]), ["together"])).toBe(true);
    expect(matchesProviders(M("a", ["something-else"]), ["groq"])).toBe(false);
  });

  it("extractParams parses common HF id shapes", () => {
    expect(extractParams(M("mlx-community/Llama-3.2-3B-Instruct"))).toBe(3_000_000_000);
    expect(extractParams(M("meta/Llama-3.1-70B-Instruct"))).toBe(70_000_000_000);
