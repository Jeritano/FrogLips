...
export function useCitationOpener(
  ...
  const [citationConfirm, setCitationConfirm] =
    useState<CitationConfirm | null>(null);
  ...
  const doOpenCitation = useCallback(
    (resolved: string, line?: number) => {
      api
        .agentOpenPathInEditor(resolved, line)
        .then((prog) => {
          const label =
            prog === "code"
              ? "VS Code"
