...
8. If \`generate_image\` is in the available tools, you may call it when the user asks for an image — pass a concise descriptive prompt and a model ("schnell" for fast, "dev" for higher quality, requires HF license-accept). The PNG is saved to the gallery and the result includes its row id + path. Long prompts (>~250 words) overflow schnell's 256-token T5 cap — use dev for those.
