...
beforeEach(() => {
  ...
  (api.agentRunShell as ReturnType<typeof vi.fn>).mockClear();
});
...
describe("workflow_invoke_skill", () => {
  ...
  // SECURITY: replay dispatches steps straight through executeTool, which
  // bypasses the runner's DANGEROUS_TOOLS → requestConfirmation gate. A step
  // that would normally prompt MUST be refused (fail-closed) rather than run
  // unattended. These pin that the gate is enforced at replay, not just at
  ...
  it("refuses a dangerous step (run_shell) at replay without executing it", async () => {
    ...
    expect(parsed.ok).toBe(false);
    expect(parsed.steps).toHaveLength(1);
    expect(parsed.steps[0].kind).toBe("forbidden_step_tool");
    expect(parsed.steps[0].tool).toBe("run_shell");
    // Neither the dangerous step nor the trailing step ran.
