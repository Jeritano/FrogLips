...
const streamMock = vi.hoisted(() => ({
  fn: vi.fn(async (seat: { name: string }, _msgs: unknown, opts: { onDelta: (d: string) => void }) => {
...
const seatA: Seat = { id: "a", name: "Optimist", color: "#0f0", backend: "openrouter", model: "x/a", system: "" };
const seatB: Seat = { id: "b", name: "Skeptic", color: "#00f", backend: "openrouter", model: "x/b", system: "" };
...
beforeEach(() => {
  ...
  streamMock.fn.mockImplementation(async (seat: { name: string }, _m: unknown, opts: { onDelta: (d: string) => void }) => {
    const text = `point from ${seat.name}`;
    opts.onDelta(text);
    return text;
  });
...
describe("runRoundtable", () => {
  it("round-robins all seats for maxRounds and ends on max_rounds", async () => {
    const res = await runRoundtable(cfg(), noopHooks, { signal: new AbortController().signal, prices: {} });
    expect(res.reason).toBe("max_rounds");
    // 2 seats × 2 rounds
    expect(res.turns.filter((t) => t.status === "done")).toHaveLength(4);
    expect(res.turns.map((t) => t.speaker)).toEqual(["Optimist", "Skeptic", "Optimist", "Skeptic"]);
  ...
  it("user Stop ends the run as 'stopped'", async () => {
    ...
    streamMock.fn.mockImplementation(async (_seat: { name: string }, _m: unknown, opts: { onDelta: (d: string) => void }) => {
      opts.onDelta("x");
      ac.abort();
      throw new DOMException("aborted", "AbortError");
    });
    const res = await runRoundtable(cfg(), noopHooks, { signal: ac.signal, prices: {} });
  ...
  it("$ budget gate refuses to start a turn that would cross the cap", async () => {
    ...
    const prices = { a: { inPerToken: 1, outPerToken: 1 }, b: { inPerToken: 1, outPerToken: 1 } };
    const res = await runRoundtable(cfg({ stop: { maxRounds: 5, maxTokens: null, maxUsd: 0.000001 } }), noopHooks, {
  ...
  it("token budget gate stops mid-run", async () => {
    const res = await runRoundtable(cfg({ stop: { maxRounds: 10, maxTokens: 50, maxUsd: null } }), noopHooks, {
      signal: new AbortController().signal,
      prices: {},
    });
  ...
  it("one failing seat is skipped; the other continues", async () => {
    ...
    const res = await runRoundtable(cfg(), noopHooks, { signal: new AbortController().signal, prices: {} });
    expect(res.reason).toBe("max_rounds");
    expect(res.turns.filter((t) => t.status === "done").map((t) => t.speaker)).toEqual(["Optimist", "Optimist"]);
  ...
  it("RT-1: a drained moderator injection reaches the next seat's prompt", async () => {
    const seen: string[] = [];
    streamMock.fn.mockImplementation(async (seat: { name: string }, msgs: unknown, opts: { onDelta: (d: string) => void }) => {
      seen.push((msgs as { content: string }[]).map((m) => m.content).join("\n"));
      opts.onDelta("ok");
      return `point from ${seat.name}`;
    });
    const mod = {
      id: "mod0", seatId: "__moderator__", speaker: "Moderator", color: "#000",
      text: "Focus on enforcement.", status: "done" as const, round: -1, kind: "moderator" as const,
    ...
    await runRoundtable(cfg({ stop: { maxRounds: 1, maxTokens: null, maxUsd: null } }), noopHooks, {
      signal: new AbortController().signal,
      prices: {},
      drainInjections,
    });
    ...
    expect(seen.some((p) => p.includes("Moderator: Focus on enforcement."))).toBe(true);
  });
});
