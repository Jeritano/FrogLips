...
- **Workflows**: an agent-orchestration canvas — a corner card deck and a table-top where you create agent cards, wire them into a left→right chain, hand each card's output to the next as input, run the whole pipeline or a single card, and schedule unattended runs. Click any edge to disconnect (confirmation prompted). Every dangerous tool call surfaces the same confirmation modal as chat agent mode; a per-run **"Auto-approve file writes"** checkbox skips the modal for normal-risk filesystem writes so a research → summary → report chain isn't punished with one modal per card (`run_shell`, `applescript_run`, `http_request`, and any write to a sensitive system / launch / dotfile path always gate explicitly). Scheduled unattended runs additionally refuse `run_shell`, `applescript_run`, `delete_path`, `kill_process`, `agent_undo`, `http_request`, `spawn_subagent`, and all MCP tools by design.
- **Images**: a built-in FLUX text-to-image surface powered by the same in-process mistralrs runtime — canvas-left layout with a vertical thumb strip and sticky bottom composer, six model variants (`schnell` / `dev` plus fp8 and GGUF Q4_K quantized siblings that run on 8 GB Macs), and a three-state gallery scope chip (All / This chat / Standalone). The detail pane offers copy-prompt / save / delete / send-to-chat, and a `generate_image` agent tool lets agents produce PNGs on demand (the agent prefers a quantized variant under memory pressure). FLUX Pro and FLUX 1.1 Pro are deliberately absent: their weights are closed-source and API-only, so they cannot run locally. First-use requires a free HuggingFace access token + license-accept on the FLUX repo; full one-time setup is documented in the [User Guide](docs/USER_GUIDE.md#one-time-huggingface-setup)
