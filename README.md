...
Plain chat is the substrate; the product is built on four pillars:

- **Workflows** — an agent-orchestration canvas: place agent cards on a table-top, wire them into a chain, hand output→input between them, and run (or schedule) multi-agent pipelines — all on the in-process native backend.
...
![version](https://img.shields.io/badge/version-0.14.34-22c55e) ![platform](https://img.shields.io/badge/platform-macOS%20Apple%20Silicon-blue) ![stack](https://img.shields.io/badge/stack-Tauri%202%20%C2%B7%20React%2019%20%C2%B7%20Rust-orange)
...
- **Light + dark + System themes**, ☀/☾ toggle in sidebar, persisted; **System** follows the OS appearance and live-updates; reduced-motion support
...
- **Roundtable**: multi-model conversations — several models (local or cloud) take turns on a shared transcript with a director/moderator, round-robin turn control, per-turn budget ($) and time caps, recent+summary memory, and graceful per-turn failure handling. The run state is lifted to App level so it survives view navigation; completed runs are saved as reusable **outcomes** (reopen, re-run, or export to file). *(In the app's sidebar this surface is labeled **Table**.)*
- **Multi-model router**: an **Auto-route** chat toggle that picks the best-fit configured model per message — keyword fast-path → semantic match (cosine over per-route example *utterances*, embedded locally) → small-LLM classifier → default. Routes bind a model/backend/role; each answer is attributed to the model that produced it. Save named, note-able **configurations** ("Hybrid cloud+local", "All-local private") and switch the whole set in one click; a **Test routing** box shows which route a message takes without sending a chat. See `docs/ROUTER_DESIGN.md`.
- **Agent presets**: General / Coder / Researcher / Shell — selectable per turn
- **Workflows**: an agent-orchestration canvas — a corner card deck and a table-top where you create agent cards, wire them into a left→right chain, hand each card's output to the next as input, run the whole pipeline or a single card, and schedule unattended runs. Each card carries its own model, role/preset, **optional custom system prompt** (overrides the preset's role for that card only), prompt, tool allowlist, and schedule. The **Run workflow / Stop** button lives in the global top-bar next to the theme toggle; per-card status renders in the right-hand status panel. Approval is unified to a single per-card **Unattended** checkbox — when ticked the card auto-approves its own allowlisted tool calls (on both manual and scheduled runs); when not, every dangerous tool call surfaces the same confirm modal as chat agent mode. There is no separate session-level toggle. Irreversible tools (`delete_path`, `kill_process`, `agent_undo`) are not selectable in the card tool picker and never auto-run. Click any edge to disconnect (confirmation prompted). A **Run History** panel records past runs of each workflow — manual or scheduled — with when they ran, status, and per-card output. *(In the app's sidebar this surface is labeled **Flows**.)*
- **MCP servers (Tools)**: a management hub to browse MCP servers from public registries (the official `registry.modelcontextprotocol.io` + PulseMCP), install **stdio** servers (`npx`/`uvx`) or add **remote** streamable-HTTP endpoints (bearer token in the macOS Keychain — `0600`-file fallback — SSRF-guarded + DNS-pinned), and view/start/stop/remove them. Connected servers' tools become available to the agent (confirmation-gated in chat; in Flows they follow the card's Unattended setting like any other tool).
...
- **Reasoning disclosure**: for thinking models (R1 / Qwen3 / gpt-oss …) the `<think>` chain-of-thought streams into a collapsible "Thinking…" disclosure that collapses to "Thought for a moment" once the answer arrives — kept out of the final prose
- **In-conversation find** (Cmd+F): a find bar scoped to the open conversation marks and steps through hits across the whole thread (including collapsed/earlier rows), with next/prev and a live match count
- **Project RAG corpora**: index a local folder so the agent can semantically search it; corpora carry a manifest (chunk count, last-indexed age), a **Stale** badge when source files change, one-click re-index, and clickable search hits that open the source file
- **Model capability badges**: the picker flags vision-capable models and a tool-calling fitness hint (good / weak / untested) plus a compact context-window marker, so you pick a model that will succeed before hitting a wall
...
- **Auto-updater**: signed minisign releases via GitHub Releases, on by default with a Settings toggle to opt out (manual "Check for updates" always available)
- **Keyboard shortcuts**: Cmd+N (new chat), Cmd+L (model library), Cmd+K (focus picker), Cmd+F (find in conversation), Cmd+, (settings) — press **?** anywhere outside a text field for the shortcuts cheatsheet
