...
- **Images**: a built-in FLUX text-to-image surface (schnell + dev) powered by the same in-process mistralrs runtime — prompt panel, scrollable gallery, detail viewer with copy-prompt / save / delete / send-to-chat, plus a `generate_image` agent tool so agents can produce PNGs on demand
